...
This root `AGENTS.md` is repository-level guidance for czclaw. Codex may also
...
czclaw is an Electron + React desktop application. Its core user-facing
...
`Cowork` is czclaw's product/session layer. The name is historical: it
started as a Claude Code-like in-house coding assistant, but in the current
codebase it means the czclaw layer that owns sessions, messages,
...
When changing OpenClaw-related behavior, first look for a czclaw-side
...
czclaw when the behavior is product-specific or can be expressed cleanly at
the integration boundary.

Use version-scoped OpenClaw patches only when the required behavior is inside
OpenClaw and there is no clean czclaw-side hook. Do not avoid a patch by
adding brittle or contorted czclaw workarounds.
...
- `src/main/libs/openclawConfigSync.ts`: renders czclaw state into
...
Important paths:
- `%APPDATA%/czclaw/openclaw/state/openclaw.json` on Windows: generated
  OpenClaw config.
- `%APPDATA%/czclaw/openclaw/state/workspace-main`: main agent workspace.
- `%APPDATA%/czclaw/openclaw/state/workspace-{agentId}`: non-main agent
  workspaces.
...
- `AGENTS.md`: OpenClaw workspace instructions with a czclaw-managed section.
...
Main logs:
- Windows: `%APPDATA%/czclaw/logs/main-YYYY-MM-DD.log`
- macOS: `~/Library/Logs/czclaw/main-YYYY-MM-DD.log`
- Linux: `~/.config/czclaw/logs/main-YYYY-MM-DD.log`

Main log retention is 7 days. Max file size is 80 MB; overflow rotates to
`.old.log`.

OpenClaw gateway capture logs:
- Windows: `%APPDATA%/czclaw/openclaw/logs/gateway-YYYY-MM-DD.log`
- Retention is 3 days.
