...
export type LogEventAction = `${typeof LogReporterActionPrefix.czclaw}${string}`;
...
const logCommons = {
  _npid: LogReporterProduct.czclaw,
...
export const reportYdAnalyzer = async (params: LogEventParams): Promise<boolean> => {
  ...
  if (!params.action.startsWith(LogReporterActionPrefix.czclaw)) {
    writeReporterLog('warn', 'skipped an event without the czclaw action prefix');
    return false;
  }
