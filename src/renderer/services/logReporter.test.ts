...
test('builds a Youdao Analyzer URL with common action parameters', () => {
  const result = new URL(buildLogUrl(
    {
      action: `${LogReporterActionPrefix.czclaw}skill_enabled`,
      skillId: 'xlsx',
      enabled: true,
    },
    {
  ...
  expect(result.searchParams.get('_npid')).toBe(LogReporterProduct.czclaw);
...
test('uses the logged-in user and omits empty optional parameters', () => {
  const result = new URL(buildLogUrl(
    {
      action: `${LogReporterActionPrefix.czclaw}app_started`,
...
test('rejects an event without the czclaw action prefix before sending', async () => {
  const fetchMock = vi.fn();
  vi.stubGlobal('window', {
    electron: {
      api: {
