...
if (!gotTheLock) {
            ...
            ? [
                {
                  ...
                  filePath: path.join(app.getPath('appData'), 'czclaw', 'install-timing.log'),
                },
              ]
            : []),
        ],
  ...
  ipcMain.handle(DataMigrationIpc.Backup, async event => {
    ...
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to back up czclaw data',
      };
    }
  });
  ...
  ipcMain.handle(DataMigrationIpc.Restore, async event => {
    ...
    try {
      ...
      return {
        ...
        error: success ? undefined : restoreResult?.error || 'Failed to import czclaw data backup',
      };
    } catch (error) {
      isCleanupInProgress = false;
      const message = error instanceof Error ? error.message : 'Failed to import czclaw data backup';
