...
import { isczclawQuotaExhaustedError } from '../../common/coworkErrorClassify';
...
function extractQuotaErrorFromProxyErrorPayload(
  ...
  try {
    ...
    if (isErrorPayload && isczclawQuotaExhaustedError(searchable)) {
      return {
    ...
    if (event === 'error' && isczclawQuotaExhaustedError(payload)) {
