...
const MANAGED_SKILL_ENTRY_OVERRIDES: Record<string, { enabled: boolean }> = {
  ...
  // czclaw configures MCP servers via openclaw.json mcp.servers field.
  ...
  // users are routed through czclaw's MCP layer instead.
  'mcporter': {
    enabled: false,
  },
};
...
const MANAGED_WEB_SEARCH_POLICY_PROMPT = [
  ...
  '- Only use the czclaw `web-search` skill when local command execution is available. Native channel sessions may deny `exec`, so prefer `browser` or `web_fetch` there.',
  ...
  '',
  'Do not claim you searched the web unless you actually used `browser`, `web_fetch`, or the czclaw `web-search` skill.',
].join('\n');
...
const MANAGED_BROWSER_POLICY_PROMPT = [
  '## Browser Policy',
  '',
  'czclaw does not support sandbox browser execution in this version.',
  '- For every `browser` tool call, set `target="host"` explicitly.',
  '- Do not use `target="sandbox"` or `target="node"` unless a future czclaw version explicitly enables it.',
 ...
 *
 * Example outputs:
 *   macOS:   ~/Library/Application Support/czclaw/SKILLs
 *   Windows: ~/AppData/Roaming/czclaw/SKILLs
 *   Linux:   ~/.config/czclaw/SKILLs
 */
...
const buildManagedSkillCreationPrompt = (skillsDirPath: string): string => [
  '## Skill Creation',
  '',
  'When the user asks you to create a new skill, you MUST place it under the czclaw skills directory:',
  '',
...
export class OpenClawConfigSync {
   ...
   * `.clobbered.<timestamp>` snapshot.  Because czclaw writes openclaw.json
...
loopDetection: MANAGED_TOOL_LOOP_DETECTION,
  ...
  sync(reason: string): OpenClawConfigSyncResult {
    ...
    const managedConfig: Record<string, unknown> = {
      ...
      ...((() => {
        ...
        const pluginEntries: Record<string, unknown> = {
          ...
          // Disable acpx (ACP agent runtime) — czclaw does not use ACP and
    ...
    if (enabledDingTalkInstances.length > 0) {
      const buildDingTalkAccountConfig = (
        ...
        // gatewayBaseUrl were czclaw-specific and are not in the plugin schema.
        dmPolicy: inst.dmPolicy || 'open',
        allowFrom: (() => {
          const ids = inst.allowFrom?.length ? [...inst.allowFrom] : [];
          if (inst.dmPolicy === 'open' && !ids.includes('*')) ids.push('*');
  ...
  /**
   * Ensures exec-approvals.json under the czclaw-managed openclaw home has
  ...
  /**
   * Resolve the czclaw SKILLs installation directory for OpenClaw's
   * `skills.load.extraDirs` configuration.
   *
   * Cross-platform paths (via Electron app.getPath('userData')):
   *   macOS:   ~/Library/Application Support/czclaw/SKILLs
   *   Windows: %APPDATA%/czclaw/SKILLs
   *   Linux:   ~/.config/czclaw/SKILLs
   */
  ...
  /**
   * Build per-skill `enabled` overrides from the czclaw SkillManager state,
   * so that skills disabled in the czclaw UI are also hidden from OpenClaw.
   */
