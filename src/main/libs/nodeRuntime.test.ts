...
describe('selectSpawnableNodeCandidate', () => {
  test('skips czclaw bash and cmd shims on Windows and returns node.exe', () => {
    expect(selectSpawnableNodeCandidate([
      'C:\\Users\\demo\\AppData\\Roaming\\czclaw\\cowork\\bin\\node',
      'C:\\Users\\demo\\AppData\\Roaming\\czclaw\\cowork\\bin\\node.cmd',
      'C:\\Program Files\\nodejs\\node.exe',
    ], 'win32', 'C:\\Users\\demo\\AppData\\Roaming\\czclaw')).toBe(
      'C:\\Program Files\\nodejs\\node.exe',
    );
  });
  ...
  test('returns null on Windows when only non-native shims are available', () => {
    expect(selectSpawnableNodeCandidate([
      'C:\\Users\\demo\\AppData\\Roaming\\czclaw\\cowork\\bin\\node',
      'C:\\Users\\demo\\AppData\\Roaming\\czclaw\\cowork\\bin\\node.cmd',
    ], 'win32', 'C:\\Users\\demo\\AppData\\Roaming\\czclaw')).toBeNull();
  });
...
describe('resolveNodeRuntimeForSpawn', () => {
  test('uses real node.exe on Windows when present after czclaw shims', () => {
    setPlatform('win32');
