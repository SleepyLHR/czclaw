 ...
 * to local Cowork sessions so that conversations are visible in the czclaw UI.
 */
...
export class OpenClawChannelSessionSync {
  ...
  resolveOrCreateSession(sessionKey: string): string | null {
    // 1. Skip czclaw-originated sessions
    if (isManagedSessionKey(sessionKey)) {
      console.log('[ChannelSessionSync] skipped: czclaw-originated session');
      return null;
    }
