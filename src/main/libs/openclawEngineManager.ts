...
export function buildOpenClawCompileCacheEnv(compileCacheDir: string): NodeJS.ProcessEnv {
  return {
    ...
    // The cache is already configured by czclaw. Prevent the packaged
...
export class OpenClawEngineManager extends EventEmitter {
  ...
  private async doStartGateway(): Promise<OpenClawEngineStatus> {
    ...
    const env: NodeJS.ProcessEnv = {
      ...
      // Disable Bonjour/mDNS LAN discovery advertising.  czclaw is a
    ...
    // find the czclaw-managed Python instead of the Windows Store stub.
  ...
  private ensureOpenClawWorkerShimsForBundle(runtimeRoot: string): void {
    try {
      ...
      if (result.protectedExisting.length > 0) {
        console.warn(
          `[OpenClaw] Skipped ${result.protectedExisting.length} worker shim(s) because existing files are not czclaw shims.`,
        );
      }
    } catch (error) {
