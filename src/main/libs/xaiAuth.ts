 ...
 * auto-refreshes it via the xai plugin's refreshOAuth hook — czclaw never
...
async function persistXaiCredential(params: {
  ...
  await withAuthStoreLock(() => {
    ...
    // czclaw models a single xAI account: drop any previous xai profiles so
...
function renderCallbackHtml(success: boolean, message: string): string {
  ...
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>czclaw · xAI Login</title>
<style>
...
function waitForCallback(expectedState: string): Promise<{ code: string }> {
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      res.end(renderCallbackHtml(true, 'You can now close this tab and return to czclaw.'));
