...
test('resolveOpenClawRuntimeErrorMessage classifies raw czclaw quota errors', () => {
...
test('outbound prompt injects continuity capsule bridge before the current request', async () => {
  ...
  expect(prompt).toContain('[czclaw continuity context after context compaction]');
  ...
  expect(prompt.indexOf('[czclaw continuity context after context compaction]')).toBeLessThan(
    prompt.indexOf('[Current user request]'),
  );
});
...
test('outbound prompt injects full capsule first and mini capsule on later turns', async () => {
  ...
  expect(firstPrompt).toContain('[czclaw continuity context after context compaction]');
  ...
  expect(secondPrompt).toContain('[czclaw brief continuity context after context compaction]');
...
test('outbound prompt injects workspace rehydration bridge before the current request', async () => {
  ...
  expect(prompt).toContain('[czclaw workspace state after context compaction]');
  expect(prompt).toContain('src/main/libs/agentEngine/coworkWorkspaceRehydration.ts');
  expect(prompt.indexOf('[czclaw workspace state after context compaction]')).toBeLessThan(
    prompt.indexOf('[Current user request]'),
  );
});
...
test('outbound prompt injects workspace rehydration bridge once per compaction', async () => {
  ...
  expect(firstPrompt).toContain('[czclaw workspace state after context compaction]');
  expect(secondPrompt).not.toContain('[czclaw workspace state after context compaction]');
});
...
test('outbound prompt injects top-k evidence bridge before the current request', async () => {
  ...
  expect(prompt).toContain('[czclaw retrieved evidence after context compaction]');
  expect(prompt).toContain('npm test failed in src/pages/Bakery.tsx');
  expect(prompt.indexOf('[czclaw retrieved evidence after context compaction]')).toBeLessThan(
    prompt.indexOf('[Current user request]'),
  );
});
...
test('outbound prompt skips continuity capsule bridge before compaction', async () => {
  ...
  expect(prompt).not.toContain('[czclaw continuity context after context compaction]');
});
...
test('subagent history sync preserves visible local user text instead of raw outbound prompt', async () => {
  const rawOutboundPrompt = `[czclaw system instructions]
hidden setup

[Context bridge from previous czclaw conversation]
previous context
