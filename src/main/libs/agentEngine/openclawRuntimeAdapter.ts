...
const buildMediaGenerationTurnInstruction = (selection?: CoworkMediaSelection, hasMediaSkillActive?: boolean): string => {
  if (!selection || selection.mode === 'none') {
    if (hasMediaSkillActive) {
      return [
        '[czclaw media generation tools — NOT AVAILABLE]',
  ...
  const lines = [
    '[czclaw media generation turn instruction]',
    'The user selected a czclaw media generation model for this turn.',
    'IMPORTANT: Do NOT read or use the "seedance" or "seedream" skills for this request.',
    'The czclaw media generation tools (lobsterai_image_generate / lobsterai_video_generate) replace those skills when a media model is selected.',
  ...
  if (!selection.imageModelId && !selection.videoModelId && selection.modelId?.trim()) {
    lines.push(`You MUST use model "${selection.modelId.trim()}" for media generation. Do NOT use a different model unless the user explicitly requests a different czclaw media model by name.`);
  }
...
const buildMediaReferencePromptSection = (mediaReferences?: CoworkMediaAttachmentRef[]): string => {
  ...
  const lines = [
    '[czclaw media reference mapping]',
  ...
  for (const ref of refs) {
    ...
    const locations = [
      ...
      !ref.localPath && !ref.remoteUrl && ref.dataUrl ? 'dataUrl fallback available through czclaw host' : '',
    ].filter(Boolean);
...
export class OpenClawRuntimeAdapter extends EventEmitter implements CoworkRuntime {
   ...
   * so czclaw can recover even when the gateway fails to deliver the abort event.
   */
   ...
   * for a czclaw-initiated session.
   */
    ...
    try {
      ...
      return {
        ...
        error: reason === CoworkSteerRejectReason.RuntimeUnsupported
          ? 'The current OpenClaw runtime does not expose same-turn steering yet. Rebuild the pinned runtime with czclaw patches.'
          : message,
      };
    }
  }
  ...
  private buildSystemPromptPrefix(systemPrompt: string): string {
    return [
      '[czclaw system instructions]',
      'Apply the instructions below as the highest-priority guidance for this session.',
      'If earlier czclaw system instructions exist, replace them with this version.',
      systemPrompt,
    ].join('\n');
  }
   ...
   * gateway rather than the czclaw UI.
   *
