...
;
; 1. czclaw.exe -- the main app AND the OpenClaw gateway (ELECTRON_RUN_AS_NODE)
; 2. node.exe whose binary lives inside the czclaw install tree
...
!macro stopczclawProcesses
  DetailPrint "[Installer] Stopping running czclaw processes"
  System::Call 'kernel32::GetTickCount()i .r7'
  nsExec::ExecToLog 'powershell -NoProfile -NonInteractive -Command "\
    Stop-Process -Name czclaw -Force -ErrorAction SilentlyContinue;\
    Get-Process node -ErrorAction SilentlyContinue | Where-Object { $$_.Path -like \"*czclaw*\" } | Stop-Process -Force -ErrorAction SilentlyContinue;\
    for ($$i = 0; $$i -lt 15; $$i++) {\
      $$procs = @();\
      $$procs += Get-Process -Name czclaw -ErrorAction SilentlyContinue;\
      $$procs += Get-Process node -ErrorAction SilentlyContinue | Where-Object { $$_.Path -like \"*czclaw*\" };\
  ...
  IntOp $5 $6 - $7
  CreateDirectory "$APPDATA\czclaw"
  FileOpen $9 "$APPDATA\czclaw\install-timing.log" a
...
!macro customInit
  ...
  CreateDirectory "$APPDATA\czclaw"
  FileOpen $9 "$APPDATA\czclaw\install-timing.log" w
  !insertmacro GetTimestamp $8
...
!macro customCheckAppRunning
  !insertmacro stopczclawProcesses

  !ifndef BUILD_UNINSTALLER
    ; -- Backup user-created skills to AppData before extraction overwrites them --
    ; Copy non-bundled skills to %APPDATA%\czclaw\skills-backup\ so they are
    ...
    System::Call 'kernel32::GetTickCount()i .r7'
    ClearErrors
    FileOpen $R0 "$APPDATA\czclaw\skill-migrate.log" w
    ...
    nsExec::ExecToStack 'powershell -NoProfile -NonInteractive -Command "\
      ...
      $$backup = \"$APPDATA\czclaw\skills-backup\";\
...
!macro customInstall
  ...
  ; Log file: %APPDATA%\czclaw\install-timing.log

  CreateDirectory "$APPDATA\czclaw"
  FileOpen $2 "$APPDATA\czclaw\install-timing.log" a
  !insertmacro GetTimestamp $8
  ...
  nsExec::ExecToLog '"$INSTDIR\${APP_EXECUTABLE_FILENAME}" "$INSTDIR\resources\unpack-cfmind.cjs" "$INSTDIR\resources\win-resources.tar" "$INSTDIR\resources" "$APPDATA\czclaw\install-timing.log"'
  Pop $0
  System::Call 'kernel32::GetTickCount()i .r6'
  IntOp $5 $6 - $7
  ...
  TarExtractProcessFailed:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "Resource extraction failed: could not start extractor process (exit=$0). This may be caused by antivirus software. See %APPDATA%\czclaw\install-timing.log for details."
    Goto TarExtractOK
  ...
  TarExtractNonZero:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "Resource extraction failed (exit code $0). See %APPDATA%\czclaw\install-timing.log for details."
  TarExtractOK:
  ...
  ; the backup.
  IfFileExists "$APPDATA\czclaw\skills-backup\*.*" 0 SkipSkillRestore
