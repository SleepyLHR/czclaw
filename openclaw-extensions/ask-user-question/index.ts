 ...
 * and waits for the user's response via an HTTP callback to czclaw.
 *
 * This enables delete-confirmation modals on the czclaw desktop app
...
const plugin = {
  ...
  description: 'Structured user confirmation tool for czclaw desktop.',
  configSchema: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // Use a factory so the tool is only available for czclaw local-session candidates.
    // IM channel sessions (qqbot, dingtalk, weixin, feishu, etc.) get null → tool hidden.
    api.registerTool((ctx) => {
      // Enable for czclaw desktop sessions across agents and delegated child sessions.
