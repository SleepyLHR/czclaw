...
1. **Configure accounts in czclaw Settings > Email.**

czclaw writes `accounts.json` for multi-account setups. Existing `.env` files are still read as a legacy single-account fallback.
...
- `accounts.json` - Multi-account credentials managed by czclaw Settings
